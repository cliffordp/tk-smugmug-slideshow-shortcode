...
if ( ! class_exists( 'TK_SmugMug_Slideshow_Shortcode' ) ) {
	...
	class TK_SmugMug_Slideshow_Shortcode {
		...
		private function val_domain( $input, $output = '' ) {
			...
			$input = str_ireplace( 'https://', '', $input );
		...
		*/
				...
				array(
					...
					'attrs'         => array(
						array(
							...
							'type'        => 'text',
							'meta'        => array(
								'placeholder' => sprintf( esc_html__( 'Example: %s', 'tk-smugmug-slideshow-shortcode' ), 'tourkick.smugmug.com' ),
							),
						),
						array(
		...
		public function shortcode( $atts, $content = '' ) {
			...
			$protocol = $this->set_protocol( $domain );
			...
			$randomize = $this->val_truefalse( $atts['randomize'] );
			$randomize = $randomize ? $randomize : apply_filters( 'tk_smugmug_slideshow_shortcode_default_randomize', 0 );
			...
			// build iframe source
			// @link http://help.smugmug.com/customer/portal/articles/93254#slideshow
			$iframe_src = sprintf( '%s://%s/frame/slideshow', $protocol, $domain );
			...
			$iframe_src = add_query_arg( $iframe_query_args, $iframe_src );

			$iframe_src = esc_url( $iframe_src );

			// start output
			// apply_filters() is used in case you want to add your own "click to play slideshow in full screen" button or something else custom

			$output = '';

			$output .= apply_filters( 'tk_smugmug_slideshow_shortcode_before_output', '', $iframe_src );
			...
			$output .= sprintf(
				'<iframe style="width: %d%s; height: %d%s" class="tk-smugmug-slideshow-shortcode %s"
				src="%s"
				width="%d"
				height="600"
				>
